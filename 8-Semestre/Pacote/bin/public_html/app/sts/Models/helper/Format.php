...
class Format
{
    ...
    public function formatCoin($number, $type = "real", $symbol = false)
    {
        $number = $this->onlyNumbers($number);

        if (strtolower($type) == "real") {
            return $number = ($symbol == TRUE ? 'R$ ' : '') . number_format($number, 2, ",", ".");
        } else {
            return $number = ($symbol == TRUE ? '$ ' : '') . number_format($number, 2, ".", ",");
        }
    }
}
